...
class TaskManager:
    ...
    def list_tasks(self):
        if not self.todo_list:
            ...
            for task in self.todo_list:
                print(f"ID: {task.task_id}, Task: {task.task}, Priority: {task.priority}, Due Date: {task.due_date}")
    ...
    def set_timer(self, task_id, seconds):
        for task in self.todo_list:
            if task.task_id == task_id:
                ...
                timer_thread = threading.Timer(seconds, self.mark_halfdone, args=[task_id])
                timer_thread.start()
                self.task_db.save_tasks(self.todo_list)
